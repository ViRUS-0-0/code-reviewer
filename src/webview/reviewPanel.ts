...
import { ReviewResult } from './sidebarProvider';
...
export class ReviewPanel {
  ...
  public static createOrShow(extensionUri: vscode.Uri, result: ReviewResult | string) {
  ...
  private constructor(panel: vscode.WebviewPanel, extensionUri: vscode.Uri, result: ReviewResult | string) {
    this._panel = panel;
  ...
  private async _openFile(file: string, line?: number) {
    ...
    if (!workspaceFolders || !file) return;
  ...
  private _update(result: ReviewResult | string) {
    this._panel.webview.html = this._getHtmlForWebview(this._panel.webview, result);
  }

  private _getHtmlForWebview(webview: vscode.Webview, result: ReviewResult | string) {
