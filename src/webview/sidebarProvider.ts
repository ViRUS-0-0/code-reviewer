...
export class SidebarProvider implements vscode.WebviewViewProvider {
  ...
  public resolveWebviewView(
    ...
    webviewView.webview.onDidReceiveMessage(async (data) => {
      ...
      switch (data.type) {
        ...
        case 'openDashboard':
          if (data.result) {
            ReviewPanel.createOrShow(this._extensionUri, data.result);
          }
          break;
        case 'copyReview':
  ...
  public setReviewResult(result: ReviewResult | string) {
    if (this._view) {
      this._view.webview.postMessage({
        type: 'reviewResult',
        result,
  ...
  public setSourceSelection(sourceType: string, sourceData: any) {
    this._currentSelection = { sourceType, sourceData };
    this._updateSourceDisplay();
  }

  private _updateSourceDisplay() {
  ...
  private async _copyReviewToClipboard(content: string) {
    try {
